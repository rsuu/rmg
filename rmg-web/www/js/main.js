import init, { ZipExtract } from "/wasm/yiiy.js";
...
(function (Dire) {
    Dire[Dire["Up"] = 0] = "Up";
    Dire[Dire["Down"] = 1] = "Down";
    Dire[Dire["Stop"] = 2] = "Stop";
})(Dire || (Dire = {}));
...
let tailScrollTop = 0;
let tail_prec = 0;
let zip_input = document.querySelector("#zip_input");
let img_list = document.querySelector("#img_list");
let btn_get = document.querySelector("#btn_get");
let btn_get_url = document.querySelector("#btn_get_url");
let tail_page = document.querySelector("#tail");
async function main() {
    await init(WASM);
...
async function event_zip_input(event) {
...
function scroll_down(zip, filelist) {
    for (const element of filelist) {
        ...
        if (tail >= head + MAX_LEN && prec > 50) {
            ...
            for (const img of Array.from(imgs)) {
                ...
                if (n == head + 1) {
                    console.debug(`n: ${n}, free-head: ${head}, tail: ${tail}, prec: ${prec}`);
                    img.remove();
                    head += 1;
                    break;
                }
...
function scroll_up(zip, filelist) {
    for (const element of filelist) {
        let win_h = document.body.scrollHeight - window.innerHeight;
        let prec = (window.pageYOffset / win_h) * 100;
        let page_number = element.page_number();
        ...
        if (tail >= head + MAX_LEN && prec < 50) {
            ...
            for (const img of Array.from(imgs)) {
                let n = parseInt(img.getAttribute("page_number"));
                if (n == tail) {
                    console.debug(`free_up: n: ${n}, head: ${head}, tail: ${tail}, prec: ${prec}`);
                    img.remove();
                    tail -= 1;
                    break;
                }
