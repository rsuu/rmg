...
impl WorkerPool {
        ...
        let pool = WorkerPool {
            state: Rc::new(PoolState {
                ...
                callback: Closure::new(|event: Event| {
                    log!("unhandled event: {}", event.type_());
                    // crate::logv(&event);
                }),
            }),
        };
    ...
    fn spawn(&self) -> Result<Worker, JsValue> {
        ...
        // Firefox does not support importing ES modules in workers
        ...
        // let worker = Worker::new("/js/worker.js")?;
        let mut opts = WorkerOptions::new();
        let opts = opts.type_(WorkerType::Module);
        let worker = Worker::new_with_options("/js/worker.js", &opts)?;
        ...
        // let buf = js_sys::Uint8Array::new(&wasm_bindgen::memory());
        // array.push(&buf);
